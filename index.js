...
class JumboApi {
  static getProduct(id) {
    if (typeof id !== 'string')
      throw new TypeError(
        `Expected id to be a string, got a ${typeof id}. Format: {number}{containerType}, e.g. 12345PAK`,
      );
  ...
  static searchProducts(query, limit, offset) {
    if (typeof query !== 'string') throw new TypeError(`Expected query to be a string, got a ${typeof query}`);
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${BASE_URL}/search?q=${query}&limit=${limit}&offset=${offset}`)
        .then((res) => {
  ...
  static searchId(productId) {
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${BASE_URL}/search?q=${productId}`)
        .then((res) => {
  ...
  static getStoreDeliveryTimeSlots(id) {
    if (typeof id !== 'number' && typeof id !== 'string')
      throw new TypeError(
        `Expected store id to be a number or a string, got a ${typeof id}. For a list of all stores use the getStore() method.`,
      );

    return new Promise((resolve, reject) => {
      axios
        .get(`${BASE_URL}/stores/slots?storeId=${id}&fulfilment=homeDelivery`)
        .then((res) => {
  ...
  static getStorePickupTimeSlots(id) {
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${BASE_URL}/stores/slots?storeId=${id}&fulfilment=collection`)
        .then((res) => {
          resolve(res.data.timeSlots.data);
        })
        .catch((err) => {
  ...
  static getStorePromotions(storeId) {
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${BASE_URL}/promotion-overview?store_id=${storeId}`)
        .then((res) => {
  ...
  static getToken(username, password) {
    if (typeof username !== 'string') throw new TypeError(`Expected username to be a string, got a ${typeof username}`);
    if (typeof password !== 'string') throw new TypeError(`Expected password to be a string, got a ${typeof password}`);
    ...
    return new Promise((resolve, reject) => {
      axios
        .post(`${BASE_URL}/users/login`, qs.stringify(body), config)
        .then((res) => {
  ...
  static getMyOrders(token) {
    if (typeof token !== 'string') throw new TypeError(`Expected token to be a string, got a ${typeof token}`);
    if (token === '') throw new Error('Please provide a token, received nothing');
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(`${BASE_URL}/users/me/orders`, config)
        .then((res) => {
          resolve(res.data);
        })
        .catch((err) => {
